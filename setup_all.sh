...
setup_vless() {
    ...
    # 运行NekoBox/sing-box设置脚本
    sudo python3 setup_nekobox_telegram.py --vless "$vless_uri"
    
    if [ $? -eq 0 ]; then
        print_success "NekoBox/sing-box代理设置完成"
    else
        print_error "NekoBox/sing-box代理设置失败"
        return 1
    fi
}
...
setup_telegram() {
    ...
    # 运行Telegram设置脚本
    sudo python3 setup_nekobox_telegram.py --bot_token "$bot_token" --chat_id "$chat_id"
...
main_menu() {
    while true; do
        ...
        echo "======================================================"
        echo "1. 设置VLESS代理 (NekoBox/sing-box)"
